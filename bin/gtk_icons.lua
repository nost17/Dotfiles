...
function icon_theme:get_icon_alt(opts)
	...
	for _, name in ipairs(name_cases) do
		local icon_info = gtk_theme:lookup_icon(name, opts.size, opts.flag)
		if icon_info then
...
local function new(theme_name, icon_size)
	...
	local itheme = crush(icon_theme, ret)
...
local main = setmetatable(icon_theme, icon_theme.mt)

local icon_info = gtk_theme:lookup_icon("firefox", 64, 0)
print(icon_info:get_filename())

-- print(main:get_icon_alt({
-- 	name = "firefox",
-- }))
