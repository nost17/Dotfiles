...
function taglist.taglist_label(t, args)
    ...
    local index_name = theme.taglist_index_as_name or false
    ...
    if not tag.getproperty(t, "icon_only") then
        ...
        if fg_color then
            text = text .. "<span color='" .. gcolor.ensure_pango_color(fg_color) ..
                "'>" .. (index_name and t.index or (gstring.xml_escape(t.name) or "")) .. "</span>"
        else
            text = text .. (index_name and t.index or (gstring.xml_escape(t.name) or ""))
        end
        text = text .. "</span>"
    end
